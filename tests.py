...
class TestBooksCollector:
    ...
    def new_book(self):
        self.new_book = 'Гордость и предубеждение и зомби'
        return self.new_book

    @pytest.fixture(autouse=True)
    def another_new_book(self):
        self.another_new_book = 'Что делать, если ваш кот хочет вас убить'
        return self.another_new_book

    @pytest.fixture(autouse=True)
    def books_with_ratings(self):
        self.books_with_ratings = {
            self.new_book: 6,
            self.another_new_book: 10,
            '9 мест, где надо побывать с мертвой принцессой': 8
        }
        return self.books_with_ratings
    ...
    def test_add_new_book_add_two_books_count_books_two(self):
        ...
        collector.add_new_book(self.new_book)
        collector.add_new_book(self.another_new_book)
    ...
    def test_get_book_rating_get_nonexistent_book_rating_rating_is_none(self):
        ...
        assert collector.get_book_rating(self.new_book) is None
    ...
    def test_get_books_with_specific_rating_add_three_new_books_one_book_with_rating_more_than_eight(self):
        ...
        for new_book, book_rating in self.books_with_ratings.items():
            collector.add_new_book(new_book)
            collector.set_book_rating(new_book, book_rating)
